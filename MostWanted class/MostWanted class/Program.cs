    ...
    public class MostWanted<T2> : IMostWanted<T2>
    {
        class MostWantedEnumerator : IEnumerator<T2>
        {
            ...
            public T2 Current
            {
                get
                {
                    if (this.anteriorMayorPeso == int.MaxValue) throw new InvalidOperationException();
                    return this.current;
                }
            }
            ...
            public bool MoveNext()
            {
                ...
                if(this.nodoMayorPeso != null)
                {
                    this.nodoMayorPeso = this.nodoMayorPeso.Siguiente;
                    while(nodoMayorPeso != null)
                    {
                        if(this.anteriorMayorPeso == this.nodoMayorPeso.Peso)
                        {
                            this.current = this.nodoMayorPeso.Valor;
                            this.mayorPeso = -1;
                            return true;
                        }
                        this.nodoMayorPeso = this.nodoMayorPeso.Siguiente;
                ...
                {
                    if (nodoActual.Peso > this.mayorPeso && nodoActual.Peso < this.anteriorMayorPeso)
                    {
                        this.current = nodoActual.Valor;
                        this.mayorPeso = nodoActual.Peso;
                        this.nodoMayorPeso = nodoActual;
                    }
                    nodoActual = nodoActual.Siguiente;
                }

                if (mayorPeso != anteriorMayorPeso && mayorPeso != -1)
                {
